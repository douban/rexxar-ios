...
@end
```
### 实现 MTReqeustHandler, MTResponseHandler, MTTaskHandler 协议
经过对 RXRViewController 的实际应用，我们发现在一个 App 内存在多个 NSURLProtocl 子类会导致难以跟踪和调试 App 中的网络请求。另外，一个请求被多个 NSURLProtocol 实例截获也会导致网络效率的降低。

因此，从 **0.4.0** 开始，我们引入了 [MTURLProtocol](https://cocoapods.org/pods/MTURLProtocol) 库，用以实现一个 App 内只有一个 NSURLProtocol 子类且所有请求只会被截获一次的逻辑。

更多使用方式，可以参考 [MTURLProtocol 文档](https://github.com/douban/MTURLProtocol/blob/master/README.md)。
...
## Changelog
- 0.4.0 使用 [MTURLProtocol](https://cocoapods.org/pods/MTURLProtocol) 替换内部的 NSURLProtocol 子类。
